        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <form method="POST" action="{{ route('tasks.store') }}" class="p-6">
                ...
                <div class="space-y-6">
                    ...
                    <input type="text" name="name" id="name" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('name') border-red-500 @enderror" required placeholder="Введите название задачи">
                        @error('name')
                    ...
                    <div>
                        ...
                        <select name="assigned_to_id" id="assigned_to_id" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
                        <option value="">Выберите исполнителя</option>
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" class="px-2 py-1 rounded bg-{{ $user->color }}-100 text-{{ $user->color }}-800"> {{ $user->name }}</option>
                            @endforeach
                        </select>
