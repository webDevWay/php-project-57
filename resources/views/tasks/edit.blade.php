        ...
        <div class="p-6 text-gray-900">
            ...
            <form method="POST" action="{{ route('tasks.update', $task) }}">
                ...
                <div class="mb-4">
                    ...
                    <select name="status_id" id="status_id" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500" >
                        ...
                        @foreach($statuses as $status)
                            <option value="{{ $status->id }}" @selected(old('status_id', $task->status_id) == $status->id) class="bg-{{ $status->color }}-100 text-{{ $status->color }}-800">{{ $status->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="labels[]" id="labels" multiple focused class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        @foreach($labels as $label)
                            <option value="{{ $label->id }}" class="bg-{{ $label->color }}-100 text-{{ $label->color }}-800" @selected(in_array($label->id, old('labels', [])))>{{ $label->name }}</option>
                        @endforeach
                    </select>
                </div>
